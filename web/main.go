...
var (
	...
	nodeSessionCache  = make(map[string]*http.Client)
...
func SaveConfig() error {
	...
	// 写入文件
	return os.WriteFile("/root/.realm/config.toml", buf.Bytes(), 0644)
}
...
func proxyToNode(node Node, method, path string, body []byte) (*http.Response, error) {
	...
	if !exists {
		...
		req, err := http.NewRequest(http.MethodPost, loginURL, bytes.NewBufferString(loginBody))
		if err != nil {
			return nil, err
		}
...
func main() {
	...
	authorized.Use(AuthRequired())
	{
		...
		authorized.GET("/api/nodes/:id/rules", func(c *gin.Context) {
			...
			resp, err := proxyToNode(node, http.MethodGet, "/get_rules?"+query.Encode(), nil)
			if err != nil {
		...
		authorized.POST("/api/nodes/:id/rules", func(c *gin.Context) {
			...
			node := nodesConfig.Nodes[id]
			resp, err := proxyToNode(node, http.MethodPost, "/add_rule", body)
			if err != nil {
		...
		authorized.DELETE("/api/nodes/:id/rules", func(c *gin.Context) {
			...
			node := nodesConfig.Nodes[id]
			resp, err := proxyToNode(node, http.MethodDelete, "/delete_rule?listen="+url.QueryEscape(listen), nil)
			if err != nil {
				c.JSON(500, gin.H{"error": "连接节点失败"})
				return
			}
		...
		authorized.POST("/api/nodes/:id/:action", func(c *gin.Context) {
			...
			node := nodesConfig.Nodes[id]
			resp, err := proxyToNode(node, http.MethodPost, path, nil)
			if err != nil {
				c.JSON(500, gin.H{"error": "连接节点失败"})
				return
			}
